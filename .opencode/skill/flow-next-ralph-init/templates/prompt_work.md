...
skip that step and proceed using the branch/review values above.

**MUST DO**:
- Use the **skill tool** to load `flow-next-work` and follow it.
- Do NOT spawn a generic task/subagent for the work step.
...
When `--review=opencode`, the work skill uses the OpenCode reviewer subagent for review.
...
**Step 3: Write impl receipt** (MANDATORY if WORK_REVIEW=rp or opencode)
For rp/opencode mode:
```bash
...
```
For opencode mode, receipt must be written via the heredoc above.
